...
import numpy as np
import math
from custom_msg.msg import FloatArray
import array as arr
...
def my_Publisher():
	rospy.init_node("thetapublisher", anonymous=True)
	joint_pub=rospy.Publisher("my_joint_states", FloatArray, queue_size=100)
	my_rate=rospy.Rate(4)
	...
	while not rospy.is_shutdown():
		...
		for i in range(2*n+2):
			if(theta1[i] > 0):
				data.custom_msg.append(float(-theta1[i]))
			else:
				data.custom_msg.append(float(theta1[i]))
			data.custom_msg.append(float(theta2[i]))
